% Load the trained NNP model (NN in Prediction configuration) with required dataset
load("../Existing_Models/NNP_model.mat");
...
% Predict the output using the trained NNP model for Trip 1
Trip_1_y_hat = predict(NNP,Trip_1_data);
...
% Trip 2 Validation
Trip_2_y_hat = predict(NNP,Trip_2_data);
Trip_2_y_actual = [Trip_2_kW_final(:,2)];

% Calculate the Mean Squared Error (MSE) for Trip 2
...
% Trip 3 Validation
Trip_3_y_hat = predict(NNP,Trip_3_data);
Trip_3_y_actual = [Trip_3_kW_final(:,2)];

% Calculate the Mean Squared Error (MSE) for Trip 3
...
% Trip 4 Validation
Trip_4_y_hat = predict(NNP,Trip_4_data);
Trip_4_y_actual = [Trip_4_kW_final(:,2)];

% Calculate the Mean Squared Error (MSE) for Trip 4
